...
import { Folder, Calendar, Download, X, QrCode } from "lucide-react";
...
export default function MediaGalleryClientPage() {
  ...
  const showExchangeQR = async (reservationId: number) => {
    try {
      const qrData = { reservationId, timestamp: Date.now(), type: "exchange_ticket" };
  ...
  return (
    <GalleryContainer>
      ...
      <FolderGrid>
        {folders.map((folder) => (
          <FolderApp key={folder.id} onClick={() => handleFolderClick(folder)}>
            <FolderIconWrapper>
              <Folder
                size={32}
                color="white"
                ...
                {userReservations.map((reservation) => (
                  <ReservationItem key={reservation.id}>
                    <ReservationThumbnail
                      src={
                        convertGoogleDriveUrl(reservation.photos?.image_url || "") ||
                    ...
                    />
                    <ReservationInfo>
                      <ReservationStatus status={reservation.status}>{reservation.status}</ReservationStatus>
                      <ReservationPhotoTitle>{reservation.photos?.title || "제목 없음"}</ReservationPhotoTitle>
                      <ReservationDetails>
                    ...
                    {reservation.status === "예약중" && (
                      <CancelButton onClick={() => cancelReservation(reservation.id)}>취소</CancelButton>
                    )}
                    ...
                    {reservation.status === "예약완료" && (
                      ...
                      >
                        <DownloadButton onClick={() => handleDownload(reservation)} disabled={downloading}>
