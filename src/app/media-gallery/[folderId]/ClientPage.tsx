...
export default function FolderGalleryClientPage() {
  ...
  const loadFolderData = async () => {
    try {
      ...
      if (response.ok) setFolder(data.folder);
      else setError(data.error || "폴더 정보를 불러오는 데 실패했습니다.");
    } catch (error) {
  ...
  const loadSubfolders = async () => {
    try {
      ...
      const data = await response.json();
      if (response.ok) setSubfolders(data.folders || []);
      else setSubfolders([]);
    } catch (error) {
    ...
    <GalleryContainer>
      <Header>
        ...
        <FolderInfo>
          ...
          {photos.length > 0 && <FolderSubtitle>{photos.length}개 사진</FolderSubtitle>}
        </FolderInfo>
        <div style={{ width: "44px" }} />
      </Header>
        ...
        <div style={{ marginBottom: "32px", maxWidth: "1000px", margin: "0 auto 32px" }}>
          ...
          <FolderGrid>
            {subfolders.map((subfolder) => (
              <SubFolderCard key={subfolder.id} onClick={() => router.push(`/media-gallery/${subfolder.id}`)}>
