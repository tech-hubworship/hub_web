...
export default function ClientPage() {
  ...
  const [photoForm, setPhotoForm] = useState<UploadFormState>({
    folder_id: 0,
    title: "",
    description: "",
    image_url: "",
  ...
  useEffect(() => {
    if (
      ...
      session?.user?.roles?.includes("사진팀")
    ) {
      loadFolders();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [status, session?.user?.isAdmin, session?.user?.roles]);
    ...
    <S.AdminLayout>
      ...
      <S.MainContent>
        <S.TopBar>
          ...
          <S.TopBarRight>
            <S.UserInfo>
              <S.UserAvatar>{session.user.name?.charAt(0) || "U"}</S.UserAvatar>
              <S.UserDetails>
