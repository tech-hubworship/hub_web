...
export async function POST(request: Request) {
  ...
  try {
    ...
    const finalBirthDate = completeGroup
      ? existingProfile?.birth_date || birth_date
      : birth_date;
    const finalGender = completeGroup ? existingProfile?.gender || gender : gender;
    const finalCommunity = completeGroup
      ? existingProfile?.community || community
      : community;
    ...
    const profileDataToUpsert = {
      ...
      status:
        roleName === "MC" || roleName === "목회자" ? "관리자" : ("활성" as const),
    ...
    if (roleName) {
      ...
      if (roleError || !roleData) {
        throw new Error(`'${roleName}' 역할을 DB에서 찾을 수 없습니다.`);
      }
    ...
    return Response.json(
      { message: "회원가입이 완료되었습니다.", user: profile },
      { status: 200 }
    );
  } catch (error: any) {
    ...
    return Response.json(
      { message: "프로필 처리 중 오류가 발생했습니다.", details: error?.message },
      { status: 500 }
    );
  }
}
