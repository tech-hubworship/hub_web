...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!day_number || day_number < 1) {
      return Response.json({ error: "유효한 이벤트 날짜가 아닙니다." }, { status: 400 });
    }
...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (group_id) query = query.eq("group_id", Number(group_id));
    if (cell_id) query = query.eq("cell_id", Number(cell_id));
    ...
    const list = safeUsers.map((u: any) => {
      ...
      return {
        ...
        meditation_created_at: meditationMap.get(u.user_id) || null,
        attendance_created_at: attendanceMap.get(u.user_id) || null,
      };
    });
