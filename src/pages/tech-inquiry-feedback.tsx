// 파일 경로: src/pages/tech-inquiry-feedback.tsx
// 로그인한 사용자가 자신의 문의사항 피드백을 확인하는 페이지
...
export default function TechInquiryFeedbackPage() {
  const router = useRouter();
  const { data: session, status: sessionStatus } = useSession();
  const { id } = router.query; // 특정 문의사항 ID (선택사항)
  ...
  useEffect(() => {
    ...
    const fetchInquiries = async () => {
      try {
        ...
        // 특정 ID가 있으면 해당 문의사항 선택
        if (id && typeof id === 'string') {
          const inquiry = result.data?.find((inq: InquiryFeedback) => inq.id === parseInt(id));
          if (inquiry) {
    ...
    <PageLayout>
      <S.Container>
        <S.Header>
          ...
          <S.Subtitle>제출하신 문의사항과 관리자 피드백을 확인하실 수 있습니다</S.Subtitle>
        </S.Header>
              ...
              <S.InquiryList>
                ...
                {inquiries.map((inquiry) => (
                  ...
                  >
                    ...
                    <S.InquiryItemDate>
                      {new Date(inquiry.created_at).toLocaleDateString('ko-KR', { timeZone: 'Asia/Seoul' })}
                    </S.InquiryItemDate>
                  </S.InquiryItem>
                ))}
              </S.InquiryList>
                ...
                <S.Section>
                  ...
                  <S.InfoGrid>
                    <S.InfoItem>
                      ...
                      <S.Badge>{INQUIRY_TYPE_LABELS[displayInquiry.inquiry_type] || displayInquiry.inquiry_type}</S.Badge>
                    </S.InfoItem>
                    <S.InfoItem>
                      <S.InfoLabel>처리 상태</S.InfoLabel>
                      <S.Badge status={displayInquiry.status}>{STATUS_LABELS[displayInquiry.status] || displayInquiry.status}</S.Badge>
                    </S.InfoItem>
                    <S.InfoItem>
                      <S.InfoLabel>제출일</S.InfoLabel>
                      <S.InfoValue>
                        {new Date(displayInquiry.created_at).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })}
                      </S.InfoValue>
                    </S.InfoItem>
                  </S.InfoGrid>
                </S.Section>
                ...
                {displayInquiry.has_response && displayInquiry.admin_response ? (
                  <S.Section>
                    <S.SectionTitle>
                      ...
                      {displayInquiry.response_at && (
                        <S.ResponseDate>
                          ({new Date(displayInquiry.response_at).toLocaleString('ko-KR', { timeZone: 'Asia/Seoul' })})
                        </S.ResponseDate>
                      )}
                    </S.SectionTitle>
                  ...
                  <S.Section>
                    ...
                    <S.WaitingBox>
                      ...
                      <S.WaitingSubtext>관리자가 검토 후 피드백을 드릴 예정입니다.</S.WaitingSubtext>
                    </S.WaitingBox>
                  </S.Section>
                )}
